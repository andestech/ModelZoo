...
def inference_FP32(model, dataloader, device):
    ...
    with torch.no_grad():
        for ii, sample in enumerate(dataloader):
            ...
            image = preprocess_test(image, labels=label, is_train=False, augment=False)
            logits = model(image)
...
def inference_FQ(model, dataloader, data_config, device, symm=True, bits=8, calibration=False):
    ...
    with torch.no_grad():
        for image, label in tqdm(dataloader):
            ...
            logits = model(image)
            pred = torch.max(logits, 1)[1].cpu().numpy()
            num_correct += np.sum(pred == label.numpy())
            num_total += image.shape[0]
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(dataloader):
            ...
            for idx in range(input_data.shape[0]):
                interpreter.set_tensor(input_details[0]["index"], np.expand_dims(input_data[idx], axis=0))
                interpreter.invoke()
                tflite_out = interpreter.get_tensor(output_details[0]["index"])
                if int(tflite_out.argmax()) == labels[idx]:
                    correct_count += 1
                data_count += 1
...
def inference_c(interpreter, dataloader, out_path):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(dataloader):
            input_data = preprocess_test(inputs, labels=labels, is_train=False, augment=False)
            input_data = input_data.numpy().transpose(0, 2, 3, 1)
            for idx in range(input_data.shape[0]):
