        ...
        if random_status >= 100:
            ...
            if random_cutout_mask_area > 0:
                ...
                self.process = lambda img: aa.distort(
                    random_cutout_or_cutout_mask(
                        tf.image.random_flip_left_right(img), img_shape, random_cutout_mask_area, random_cutout=0
                    )
                )
            else:
                aa.available_ops = [
            ...
            if random_cutout_mask_area > 0:
                self.process = lambda img: self.tf_buildin_image_random(
                    random_cutout_or_cutout_mask(img, random_cutout_mask_area, random_cutout=0)
                )
            else:
...
def prepare_dataset(
    ...
    print(image_names_reg)
    print(image_classes_rule)
    ...
    if total_images == 0:
        ...
        return None, None
    print(
        ">>>> Image length: %d, Image class length: %d, classes: %d" % (len(image_names), len(image_classes), classes)
    )
    ...
    if len(embeddings) != 0 and teacher_model_interf is None:
        ...
        ds = tf.data.Dataset.from_tensor_slices((image_names, embeddings, image_classes)).shuffle(
    ...
    def __init__(
        ...
        **kw,
        ...
        self.split_func = lambda xx: np.array(
            np.split(
                np.random.permutation(xx)[: len(xx) // image_per_class * image_per_class], len(xx) // image_per_class
            )
        )
...
class Triplet_dataset_offline:
    ...
    def offline_triplet_mining(self):
        ...
        ds = (
            ds.map(lambda xx: (tf_imread(xx) - 127.5) * 0.0078125).batch(self.batch_size).prefetch(buffer_size=AUTOTUNE)
        )
