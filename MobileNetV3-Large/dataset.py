...
def return_dataset():
    ...
    cal_dataset = Subset(tra_dataset, cal_samples) #extract specific calibration subset for quantiztation
    ...
    tra_dataloader = DataLoader(tra_dataset, batch_size=input_yaml['batch_size'], shuffle=True, num_workers=4, pin_memory=True)
    val_dataloader = DataLoader(val_dataset, batch_size=input_yaml['batch_size'], shuffle=False, num_workers=4, pin_memory=False)
    cal_dataloader = DataLoader(cal_dataset, batch_size=input_yaml['batch_size'], shuffle=False, num_workers=4, pin_memory=True)
...
def prepare_testbin(interpreter, dataloader, save_path="./output/"):
    ...
    correct_count = 0
    data_count = 0
    for inputs, labels in tqdm(dataloader, desc="Generating test_bin"):
