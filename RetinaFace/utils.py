...
def pad_input_image(img, max_steps):
    ...
    img_pad_h = max(img_h, img_w) - img_h
    img_pad_w = max(img_h, img_w) - img_w
...
def recover_pad_output(outputs, pad_params):
    ...
    recover_xy = np.reshape(outputs[:, :14], [-1, 7, 2]) * np.concatenate(
        [(img_pad_w + img_w) / img_w, (img_pad_h + img_h) / img_h], axis=-1
    )
...
def draw_bbox_landm(img, ann, img_height, img_width):
    ...
    cv2.putText(
        img, text, (int(ann[0] * img_width), int(ann[1] * img_height)), cv2.FONT_HERSHEY_DUPLEX, 0.5, (255, 255, 255)
    )
