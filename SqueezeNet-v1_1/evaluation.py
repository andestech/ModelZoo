...
def inference_Backend(interpreter, dataloader, data_config, device, symm=True, bits=8):
    ...
    in_qmin = np.iinfo(in_dtype).min
    in_qmax = np.iinfo(in_dtype).max
    ...
    for inputs, labels in tqdm(dataloader):
        for i in range(inputs.shape[0]):
            if len(inputs[i].unsqueeze(0).numpy().shape) == 4:
                ...
                input_data.div_(input_scale).round_().add_(input_zp).clamp_(in_qmin, in_qmax)
                input_data = in_dtype(input_data.numpy().transpose(0, 2, 3, 1))
