...
def get_data(root, batch_size):
    ...
    dataset = MFCC_Loader(
        root + "librispeech_full_size", root + "librispeech_reduced_size", root + "fluent_speech_commands_dataset"
    )

    return {
        "train_full_size": [
            dataset.full_training_set(batch_size=batch_size, num_samples=132553).with_options(options),
            dataset.num_steps(batch=batch_size),
        ],
        "train_reduced_size": [
            dataset.reduced_training_set(batch_size=batch_size, num_samples=132553).with_options(options),
            dataset.num_steps(batch=batch_size),
        ],
        "val_full_size": [
            dataset.full_validation_set(batch_size=1).with_options(options),
            dataset.num_steps(batch=1),
        ],
        "val_reduced_size": [
            dataset.reduced_validation_set(batch_size=1).with_options(options),
            dataset.num_steps(batch=1),
        ],
        "train_fluent_speech": [
            dataset.fluent_speech_train_set(batch_size=batch_size, num_samples=23132).with_options(options),
            dataset.num_steps(batch=batch_size),
        ],
        "val_fluent_speech": [
            dataset.fluent_speech_validation_set(batch_size=1).with_options(options),
            dataset.num_steps(batch=1),
        ],
        "test_fluent_speech": [
            dataset.fluent_speech_test_set(batch_size=1).with_options(options),
            dataset.num_steps(batch=1),
        ],
    }
...
def train_model(args):
    ...
    if args.with_baseline:
        ...
        with strategy.scope():
            opt = tf.keras.optimizers.Adam(
                learning_rate=get_lr_schedule(learning_rate=1e-4, steps_per_epoch=full_training_num_steps)
            )
    ...
    if args.with_finetuning:
        ...
        with strategy.scope():
            opt = tf.keras.optimizers.Adam(
                learning_rate=get_lr_schedule(learning_rate=1e-5, steps_per_epoch=reduced_training_num_steps)
            )
            model.compile(loss=get_metrics("loss"), metrics=[get_metrics("ler")], optimizer=opt)
    ...
    if args.with_fluent_speech:
        ...
        with strategy.scope():
            opt = tf.keras.optimizers.Adam(
                learning_rate=get_lr_schedule(learning_rate=1e-5, steps_per_epoch=fluent_speech_training_num_steps)
            )
