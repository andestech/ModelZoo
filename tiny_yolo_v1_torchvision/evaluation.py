...
import numpy as np
import pathlib
from .utils import decode_v1, evaluate_map_voc, detect, detect_tflite
...
"""
def inference_FQ(net, dataloader, data_config, device, symm=True, bits=8):
    import os
...
def inference_HW(net, dataloader, data_config, device, symm=True, bits=8):
    import os
    ...
    with torch.no_grad():
        ...
        for id in tqdm(range(len(dataloader.dataset))):
            id = torch.tensor(id).to(device)
            input, label = dataloader.dataset[id]
            QuantStub(input, data_config["fp32_min"], data_config["fp32_max"], symm, bits, isHW=True)
            boxes_detected, classes_detected, probs_detected = detect(
                input.to(device), model, decode, 0.5, num_classes=20, prob_threshold=0.01
            )
            target_bboxes = label[0].to(device)
...
def inference_Backend(interpreter, dataloader, data_config, device, symm=True, bits=8):
    ...
    for id in tqdm(range(len(dataloader.dataset))):
        ...
        if len(input.numpy().shape) == 3:
            ...
            box_f = box_f.reshape(1, 30, 7, 7)
            box_f = box_f.permute(0, 2, 3, 1)
